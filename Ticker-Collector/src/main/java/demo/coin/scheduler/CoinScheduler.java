        ...
        if (marketOrderList.size() >= 10) {
            ...
            for (Orderbook ob : orderbookList) {
                ...
                if (money > 0
                        ...
                        && unit.getAskPrice().compareTo(targetMap.get(ob.getMarket()).getTargetPrice().multiply(BigDecimal.valueOf(1.003))) < 0
                ...
                if (isCoinBuy && unit.getBidPrice().compareTo(targetMap.get(ob.getMarket()).getTargetPrice().multiply(BigDecimal.valueOf(1.30))) > 0) {
                    sendSlackHook(SlackMessage.builder()
                ...
                if (isCoinBuy && unit.getBidPrice().compareTo(ma15) < 0 && unit.getBidPrice().compareTo(targetMap.get(ob.getMarket()).getTargetPrice().multiply(BigDecimal.valueOf(1.003))) > 0) {
                    sendSlackHook(SlackMessage.builder()
...
//    }
    ...
    List<MinuteCandle> getMinuteCandle(String marketName, int count) throws JsonProcessingException {
        WebClient client = WebClient.create("https://api.upbit.com/v1");
