...
public interface DayCandleRepository extends JpaRepository<DayCandle, DayCandleKey> {
    ...
    @Query(value = "select dc from DayCandle dc where dc.candleDateTimeUtc >= :startDate and dc.candleDateTimeUtc < :endDate and dc.market like 'KRW-%' order by (dc.tradePrice - dc.openingPrice) / dc.openingPrice desc")
